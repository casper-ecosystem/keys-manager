...
pub extern "C" fn set_key_weight() {
    let key: AccountHash = get_arg(ARG_ACCOUNT);
    let weight: Weight = Weight::new(get_arg(ARG_WEIGHT));
    update_key_weight(key, weight);
}

#[no_mangle]
pub extern "C" fn set_deployment_threshold() {
    let threshold: Weight = Weight::new(get_arg(ARG_WEIGHT));
...
pub extern "C" fn set_all() {
    let deployment_thereshold: Weight = Weight::new(get_arg(ARG_DEPLOYMENT_THRESHOLD));
    let key_management_threshold: Weight = Weight::new(get_arg(ARG_KEY_MANAGEMENT_THRESHOLD));
    let accounts: Vec<AccountHash> = get_arg(ARG_ACCOUNTS);
    let weights: Vec<Weight> = get_arg(ARG_WEIGHTS);
...
pub extern "C" fn call() {
    ...
    entry_points.add_entry_point(EntryPoint::new(
        ...
        vec![
            ...
            Parameter::new(ARG_ACCOUNTS, CLType::List(Box::new(AccountHash::cl_type()))),
...
fn get_arg<T: CLTyped + FromBytes>(name: &str) -> T {
    runtime::get_named_arg(name)
}
